...
calculateCategoryValues <- function(dataset){ #função auxiliar
  categories <- unique(dataset$category)
  v <- rep(0, length(categories))
  names(v) <- categories
  
  for(i in 1:length(v)){
    v[i] <- median(dataset[dataset$category == categories[i], "usd_pledged_real"]) #mediana para não ser influenciado por outliers
  }
  
  return(v)
}

createCategory2 <- function(dataset, factorNr){
  v <- calculateCategoryValues(dataset) 
  
  bestCategories <- factor(names(tail(sort(v), factorNr))) #definir aqui o número de subcategorias
  
  r <- rep("Other", dim(dataset)[1])
  for (i in 1:length(r)){
    if (dataset$category[i] %in% bestCategories){
      r[i] <- as.character(dataset$category[i])
    }
  }
  
  return(factor(r))
}
projects$category2 <- createCategory2(projects, 10)

###Transformação das subcategorias nas medianas dos seus pledges
createCategory3 <- function(dataset){
  v <- calculateCategoryValues(dataset) 
  
  return(v[dataset$category])
}

projects$category3 <- createCategory3(projects)
